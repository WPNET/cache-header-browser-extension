...
Reference copy for the Cache Captain listing on addons.mozilla.org.

> Note: The Firefox port intentionally omits the **Cache storage** clearing option.
> Firefox's `browsingData` API does not support the `cacheStorage` data type
> (it is Chrome-only), so it is not offered here.

## Summary (short description)

Max 250 characters on AMO.
...
Paste into the AMO **Description** field.
...
ONE-CLICK CLEARING
Choose exactly what to clear — Cache, Cookies, Local storage, IndexedDB, Service workers — over a time range (last hour / 24 hours / 7 days / all time) and scope (all domains or current site only). Optionally show a confirmation notification, and optionally force a hard reload that bypasses the server cache.
...
- Primary: Other / Developer Tools

## Add-on ID

```
cache-captain@gbotica
```

## Data collection (Firefox built-in consent)

Declared in `manifest.json` as `browser_specific_settings.gecko.data_collection_permissions.required = ["none"]`.
...
| `<all_urls>` (host permission) | The cache header can appear on any HTTP/HTTPS site the user visits, so the header must be readable across all such hosts. No page content is read. |
